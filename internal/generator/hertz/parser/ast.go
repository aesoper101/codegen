...
func astToService(ast *parser.Thrift, resolver *Resolver, args *config.Argument) ([]*generator.Service, error) {
	...
	for _, s := range ss {
		...
		service.BaseDomain = ""
		domainAnno := getAnnotation(s.Annotations, thrift.ApiBaseDomain)
		if len(domainAnno) == 1 {
		...
		groupAnno := getAnnotation(s.Annotations, thrift.ApiServiceGroup)
		if len(groupAnno) == 1 {
			if args.CmdType != meta.CmdClient {
		...
		serviceGenDirAnno := getAnnotation(s.Annotations, thrift.ApiServiceGenDir)
		...
		servicePathAnno := getAnnotation(s.Annotations, thrift.ApiServicePath)
		servicePath := ""
		if len(servicePathAnno) > 0 {
			servicePath = servicePathAnno[0]
		}
		for _, m := range ms {
			rs := getAnnotations(m.Annotations, thrift.HttpMethodAnnotations)
			if len(rs) == 0 {
				continue
			}
			...
			genPaths := getAnnotation(m.Annotations, thrift.ApiGenPath)
			if len(genPaths) == 1 {
			...
			sr, _ := util.GetFirstKV(getAnnotations(m.Annotations, thrift.SerializerTags))
...
func parseAnnotationToClient(clientMethod *generator.ClientMethod, p *parser.Type, symbol ResolvedSymbol) error {
	...
	for _, field := range st.Fields() {
		...
		if anno := getAnnotation(field.Annotations, thrift.AnnotationQuery); len(anno) > 0 {
			hasAnnotation = true
			query := checkSnakeName(anno[0])
			clientMethod.QueryParamsCode += fmt.Sprintf("%q: req.Get%s(),\n", query, field.GoName().String())
		}

		if anno := getAnnotation(field.Annotations, thrift.AnnotationPath); len(anno) > 0 {
			hasAnnotation = true
		...
		if anno := getAnnotation(field.Annotations, thrift.AnnotationHeader); len(anno) > 0 {
			hasAnnotation = true
		...
		if anno := getAnnotation(field.Annotations, thrift.AnnotationForm); len(anno) > 0 {
			hasAnnotation = true
		...
		if anno := getAnnotation(field.Annotations, thrift.AnnotationBody); len(anno) > 0 {
			hasAnnotation = true
			hasBodyAnnotation = true
		}

		if anno := getAnnotation(field.Annotations, thrift.AnnotationFileName); len(anno) > 0 {
			hasAnnotation = true
